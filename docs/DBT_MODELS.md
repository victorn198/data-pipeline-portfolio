...
-   `fct_sales.sql`: Tabela de fatos principal no grao de uma linha de transacao (`order_line_id`). Mantem `order_id` como identificador do pedido/fatura, `gross_sales_amount`, `sales_amount` liquido de cancelamento, `cancelled_sales_amount`, `quantity`, `unit_price` e chaves para as dimensoes `customer` e `product`.
